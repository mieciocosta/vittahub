...
export default function Inbox({ onUnreadChange }) {
  ...
  // ── POLL PRINCIPAL de mensagens (1.5s) — robusto, não depende de SSE ──────
  // Railway pode buffenizar SSE; este poll garante que mensagens chegam mesmo assim
  useEffect(() => {
    if (!sel) return;
    const iv = setInterval(async () => {
      ...
      try {
        ...
        const data = await fetch(
          `${import.meta.env.VITE_API_URL||''}/api/inbox/conversations/${sel.id}/messages/new?after_ts=${encodeURIComponent(afterTs)}`,
  ...
  // ── POLL da lista de conversas (5s — incremental) ─────────────────────────
  useEffect(() => {
    const iv = setInterval(async () => {
      if (!isTabActive()) return;
      try {
