...
Qual delas te trouxe aqui hoje?`]).catch(() => {});
    ...
    try {
      ...
      if (fs.existsSync(dirFig)) {
        ...
        /* Só as 14 ORIGINAIS aqui. As novas (nomeadas "categoria__frase.webp")
           são carregadas mais abaixo, com a categoria certa — se as duas
           rotinas pegassem os mesmos arquivos, a Biblioteca duplicaria tudo. */
        for (const f of fs.readdirSync(dirFig).filter(x => x.endsWith('.webp') && !x.includes('__'))) {
    ...
    /* 🏅 PRÊMIO v3 (ajuste do master, 22/08): Raylane passa de 2.500 pra
       R$ 2.000. O recado de parabéns continua. */
    const { rows: [flagPremioV3] } = await query("SELECT 1 FROM configuracoes WHERE chave = 'seed_premio_raylane_v3'");
    if (!flagPremioV3) {
      await query(`UPDATE configuracoes SET
                     valor = COALESCE(valor,'{}'::jsonb) || '{"premiosPessoa":{"raylane":2000}}'::jsonb,
                     updated_at = NOW()
                   WHERE chave = 'metas'`).catch(() => {});
      await query(`INSERT INTO configuracoes (chave, valor) VALUES ('seed_premio_raylane_v3','{"ok":true}') ON CONFLICT DO NOTHING`);
  ...
  // 🔁 Versão 3: texto novo da Dra. Nágila (ordem do master, 19/09)
  try {
    const { rowCount } = await query(`INSERT INTO configuracoes (chave, valor) VALUES ('festa_versao3_2026-09-19', '{"ok":true}') ON CONFLICT DO NOTHING`);
    if (rowCount) {
      await query(`UPDATE configuracoes SET valor = valor || '{"versao":3,"lidas":{}}'::jsonb, updated_at = NOW()
        WHERE chave = 'festa_ativa' AND valor->>'id' LIKE 'vacinas-2026-09-19-%'`);
    }
...
async function limparCarteirasFechadas() {
  const FLAG = 'seed_limpar_carteiras_fechadas_v4';   // v4 (04/09): + Agenda espontâneo, transmissão, comunidade
  const SEM_ACENTO = "'áàâãäéèêëíìîïóòôõöúùûüç','aaaaaeeeeiiiiooooouuuuc'";
  const { rows: [ja] } = await query('SELECT 1 FROM configuracoes WHERE chave = $1', [FLAG]).catch(() => ({ rows: [1] }));
  if (ja) return;
  const { rowCount } = await query(
    `UPDATE conversas c SET responsavel_id = NULL, responsavel_desde = NULL
     ...
     WHERE u.id = c.responsavel_id
       ...
       AND (COALESCE(c.contact_id,'') ~* 'g\\.us|@broadcast|@lid|@newsletter'
            OR length(regexp_replace(COALESCE(c.phone,''), '\\D', '', 'g')) > 13
            OR lower(translate(COALESCE(c.contact_name,''), ${SEM_ACENTO})) ~ '(^|[^a-z])(dr|dra)\\.?([^a-z]|$)|miecio|nagila|felipe|motorista|^\\s*ativos?\\s*$|^\\s*home\\s*$|agenda\\s*espont|espontane')`)
