/* ─── 🧹 BOAS-VINDAS DIÁRIAS — DESATIVADO ────────────────────────────────────
   Componente aposentado (pedido do master: "veja o que não está sendo usado e
   retira do sistema, mas deixa só comentado no código").

   Por que saiu: não era importado por tela nenhuma — virou código morto quando
   a saudação e o versículo do dia passaram a viver no próprio Resumo
   (Dashboard) e na barra lateral, onde a equipe realmente olha. Manter dois
   lugares dando bom-dia só criava a chance de eles se contradizerem.

   Fica aqui inteiro, comentado: se um dia o master quiser o pop-up de boas
   -vindas de volta, é só descomentar e importar no App.
   ATENÇÃO: a lista de versículos oficial é `frontend/src/hooks/versiculos.js`
   (espelhada em `backend/src/versiculos.js`) — a lista abaixo é uma cópia
   velha e menor, não use como fonte.                                          */
