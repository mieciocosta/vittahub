...
const INTERESSES_FAIXA = ['Vacina','Plano Vacinal','Consulta','Terapia','Plano Infantil','Gestante','Cliente ativo vacinas','Cliente ativo terapia','Outro'];
...
import { BotaoChatEquipe, PainelChatEquipe } from '../components/ChatEquipe.jsx';
...
export default function Inbox({ onUnreadChange }) {
  ...
  useEffect(() => {
    ...
    import('socket.io-client').then(({ io }) => {
      ...
      /* 💬 Chat da equipe: o socket é um só (deste componente), então a mensagem
         é repassada como evento da janela — o botão e o painel escutam de onde
         estiverem, sem abrir uma segunda conexão. */
      socket.on('chat_equipe_msg', (m) => window.dispatchEvent(new CustomEvent('vh_chat_equipe', { detail: m })));
      socket.on('chat_equipe_del', (d) => window.dispatchEvent(new CustomEvent('vh_chat_equipe_del', { detail: d })));
  ...
  return (
    <div className="vh-inbox-wrap" style={{ display:'flex', height:'100vh', overflow:'hidden' }}
      ...
      <div ref={listaRef} className={`vh-inbox-list${sel ? ' hidden' : ''}`}
        ...
        // A gaveta do chat da equipe abre DENTRO desta coluna, não por cima da tela
        position:'relative' }}>
        {/* Header */}
          ...
          {showProntas && sel && (() => {
            ...
            const GRUPOS_PRONTAS = [
              ...
              ['✅ Cartões de agendamento (preencha os campos)', [
                ['Vacina em casa', cartao(['✅ Vacinação confirmada','',`📁 Cliente: ${nomeC}`,`👶🏻 Paciente: ${pac}`,'📅 Data: {DATA} {DIA DA SEMANA}','🕓 Horário: {HORA}hs','📍 Local: Em sua residência','📌 Serviço: Vacinas de {IDADE}','','Vittalis Saúde cuidando do que mais importa 🩵','',INSTA])],
                ['Vacina na clínica', cartao(['✅ Vacinação confirmada','',`📁 Cliente: ${nomeC}`,`👶🏻 Paciente: ${pac}`,'📅 Data: {DATA} {DIA DA SEMANA}','🕓 Horário: {HORA}hs','📍 Local: Na Clínica Vittalis Saúde (Renascença)','📌 Serviço: Vacinas de {IDADE}','',ENDER,'','Vittalis Saúde cuidando do que mais importa 🩵','',INSTA])],
                ['Consulta na clínica', cartao(['✅ Consulta confirmada','',`📁 Cliente: ${nomeC}`,`👶🏻 Paciente: ${pac}`,'📅 Data: {DATA} {DIA DA SEMANA}','🕓 Horário: {HORA}hs','👩‍⚕️ Profissional: {PROFISSIONAL} ({ESPECIALIDADE})','📍 Local: Na Clínica Vittalis Saúde (Renascença)','📌 Serviço: {SERVIÇO}','',ENDER,'','Vittalis Saúde cuidando do que mais importa 🩵','',INSTA])],
                ['Consulta em casa', cartao(['✅ Consulta confirmada','',`📁 Cliente: ${nomeC}`,`👶🏻 Paciente: ${pac}`,'📅 Data: {DATA} {DIA DA SEMANA}','🕓 Horário: {HORA}hs','👩‍⚕️ Profissional: {PROFISSIONAL} ({ESPECIALIDADE})','📍 Local: Em sua residência','📌 Serviço: {SERVIÇO}','','Vittalis Saúde cuidando do que mais importa 🩵','',INSTA])],
              ]],
            ];
            return (
